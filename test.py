...
while True:
    ...
    for i in range(len(masks)):
        ...
        # 객체 크기를 두 배로 확대
        obj_large = cv2.resize(obj_cropped, None, fx=2, fy=2, interpolation=cv2.INTER_LINEAR)
        mask_large = cv2.resize(mask_cropped, None, fx=2, fy=2, interpolation=cv2.INTER_LINEAR)
